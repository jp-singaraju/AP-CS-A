...
{    
    ...
    {
        place(getObjectAt(oldLocation), newLocation);
        remove(oldLocation);
    }
    ...
    public List<Location> adjacentLocations(Location location)
    {
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                int rowWithin = Math.abs(locRow - row);
                int colWithin = Math.abs(locCol - col);
                if (rowWithin == 0 || rowWithin == 1) {
                    if (colWithin == 0 || colWithin == 1) {
                        Location otherLocation = new Location(row, col);
                        if (location.equals(otherLocation) == false) {
                            myList.add(otherLocation);
                        }
                    }
                }
            }
    ...
    {
        List<Location> myList = new ArrayList<> ();
        myList = adjacentLocations(location);
